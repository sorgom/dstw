...
namespace test
{
    ...
    TEST(TSW_01, T00)
    {
        cout << '\n';
        STEP(1)
        m_Dispatcher().expectDispatch(mId, ComTeleGui(TSW_STATE_LEFT));
        mSUT.process(ComTeleFld(TSW_STATE_LEFT));
        CHECK_N_CLEAR()

        STEP(2)
        m_Dispatcher().expectDispatch(mId, ComTeleGui(12));
        {
            const ComTeleGui tel(TSW_GUI_GMD_RIGHT);
            mSUT.process(tel);
        }
        CHECK_N_CLEAR()
    }
